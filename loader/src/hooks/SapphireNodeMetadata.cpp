...
class SapphireNodeMetadata final : public cocos2d::CCObject {
private:
    ...
    SapphireNodeMetadata() : m_fieldContainer(new FieldContainer()) {}

    virtual ~SapphireNodeMetadata() {
        delete m_fieldContainer;
    }

public:
    static SapphireNodeMetadata* set(CCNode* target) {
        ...
        if (old && old->getTag() == METADATA_TAG) {
            return static_cast<SapphireNodeMetadata*>(old);
        }
        auto meta = new SapphireNodeMetadata();
        meta->autorelease();
...
struct ProxyCCNode : Modify<ProxyCCNode, CCNode> {
    virtual CCObject* getUserObject() {
        ...
        return SapphireNodeMetadata::set(this)->m_userObject;
    }
    virtual void setUserObject(CCObject* obj) {
        SapphireNodeMetadata::set(this)->m_userObject = obj;
    }
};
...
FieldContainer* CCNode::getFieldContainer() {
    return SapphireNodeMetadata::set(this)->getFieldContainer();
}

std::string CCNode::getID() {
    return SapphireNodeMetadata::set(this)->m_id;
}

void CCNode::setID(std::string const& id) {
    SapphireNodeMetadata::set(this)->m_id = id;
}
...
void CCNode::setLayout(Layout* layout, bool apply, bool respectAnchor) {
    ...
    SapphireNodeMetadata::set(this)->m_layout = layout;
    if (apply) {
        this->updateLayout();
    }
}
...
Layout* CCNode::getLayout() {
    return SapphireNodeMetadata::set(this)->m_layout.data();
}

void CCNode::setLayoutOptions(LayoutOptions* options, bool apply) {
    SapphireNodeMetadata::set(this)->m_layoutOptions.reset(options);
    if (apply && m_pParent) {
...
LayoutOptions* CCNode::getLayoutOptions() {
    return SapphireNodeMetadata::set(this)->m_layoutOptions.get();
}
...
void CCNode::updateLayout(bool updateChildOrder) {
    ...
    if (auto layout = SapphireNodeMetadata::set(this)->m_layout.data()) {
        layout->apply(this);
    }
}
...
void CCNode::setAttribute(std::string const& attr, json::Value const& value) {
    auto meta = SapphireNodeMetadata::set(this);
...
void CCNode::removeEventListener(std::string const& id) {
    SapphireNodeMetadata::set(this)->m_idEventListeners.erase(id);
}
...
size_t CCNode::getEventListenerCount() {
    return SapphireNodeMetadata::set(this)->m_idEventListeners.size() +
        SapphireNodeMetadata::set(this)->m_eventListeners.size();
}
