...
#define $on_mod(type) \
template<class>                                                        \
void GEODE_CONCAT(sapphireExecFunction, __LINE__)(sapphire::ModStateEvent*); \
namespace {                                                            \
	struct GEODE_CONCAT(ExecFuncUnique, __LINE__) {};                  \
}                                                                      \
static inline auto GEODE_CONCAT(Exec, __LINE__) = (new sapphire::EventListener(  \
            &GEODE_CONCAT(sapphireExecFunction, __LINE__)<GEODE_CONCAT(ExecFuncUnique, __LINE__)>, \
            sapphire::ModStateFilter(sapphire::getMod(), sapphire::ModEventType::type) \
        ), 0);                                                         \
template<class>                                                        \
void GEODE_CONCAT(sapphireExecFunction, __LINE__)(sapphire::ModStateEvent*)
// clang-format on
