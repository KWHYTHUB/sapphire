...
namespace sapphire {
    ...
    constexpr char const* IPC_PIPE_NAME = "\\\\.\\pipe\\SapphireIPCPipe";
    #endif

    #ifdef GEODE_IS_MACOS
    constexpr char const* IPC_PORT_NAME = "SapphireIPCPipe";
    #endif

    class IPCFilter;

    // IPC (Inter-Process Communication) provides a way for Sapphire mods to talk 
    ...
    // example, an external application can query what mods are loaded in Sapphire 
    // by sending the `list-mods` message to `sapphire.loader`.
