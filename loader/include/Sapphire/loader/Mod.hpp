...
namespace sapphire {
    ...
    protected:
        ...
        // used internally in sapphire_implicit_load
        template <class = void>
    ...
    public:
        ...
        /**
         * Disable this mod (if supported), then delete the mod's .sapphire package.
         * @returns Successful result on success,
         * errorful result with info on error
         */
        Result<> uninstall();
...
GEODE_HIDDEN inline char const* operator"" _spr(char const* str, size_t) {
    return sapphire::Mod::get()->expandSpriteName(str);
}
