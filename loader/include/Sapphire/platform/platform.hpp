...
// Windows
    ...
    static_assert(sizeof(void*) == 4, "Sapphire must be compiled in 32-bit for Windows!");
...
namespace std {
    template <>
    struct hash<sapphire::PlatformID> {
        inline std::size_t operator()(sapphire::PlatformID const& id) const {
            return std::hash<sapphire::PlatformID::Type>()(id.m_value);
        }
    };
}
