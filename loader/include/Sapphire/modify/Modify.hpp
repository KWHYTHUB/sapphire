...
namespace sapphire::modifier {
    ...
    public:
        ModifyDerive() {
            static_assert(
                alwaysFalse<Derived>,
                "Modified class not recognized, please include <Sapphire/modify/ClassName.hpp> to be "
                "able to use it."
            );
        }
    };
...
#define GEODE_MODIFY_DECLARE_ANONYMOUS(base, derived) \
    ...
    template <>                                       \
    struct GEODE_HIDDEN derived<derived##Parent> : sapphire::Modify<derived<derived##Parent>, base>

#define GEODE_MODIFY_DECLARE(base, derived) \
    derived##Dummy;                         \
    struct GEODE_HIDDEN derived : sapphire::Modify<derived, base>
...
#define GEODE_MODIFY_DECLARE(base, derived) \
    derived##Dummy; \
    struct derived : sapphire::Modify<derived, base>
