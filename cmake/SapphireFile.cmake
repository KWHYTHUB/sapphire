...
if (NOT DEFINED GEODE_CLI)
    find_program(GEODE_CLI NAMES sapphire.exe sapphire-cli.exe sapphire sapphire-cli PATHS ${CLI_PATH})
endif()

# Check if CLI was found
if (GEODE_CLI STREQUAL "GEODE_CLI-NOTFOUND")
	message(WARNING "Unable to find Sapphire CLI")
else()
    # `sapphire --version` returns `sapphire x.x.x\n` so gotta do some wacky shit
    execute_process(
    ...
    # Remove program name
    string(REPLACE "sapphire " "" GEODE_CLI_VERSION ${GEODE_CLI_VERSION})

    # Need at least v1.0.5 (--shut-up arg in sapphire package resources)
    if (${GEODE_CLI_VERSION} VERSION_LESS ${GEODE_CLI_MINIMUM_VERSION})
        message(FATAL_ERROR
            "Found Sapphire CLI: ${GEODE_CLI}, however it is version ${GEODE_CLI_VERSION} "
            "while minimum required is version ${GEODE_CLI_MINIMUM_VERSION}. Please update: "
            "https://github.com/KWHYTHUB/cli/releases/latest"
        )
    endif()
    ...
    message(STATUS "Found Sapphire CLI: ${GEODE_CLI} (version ${GEODE_CLI_VERSION})")
endif()
...
# calling setup_sapphire_mod if the mod depends on external dependencies that 
# aren't being built
function(setup_sapphire_mod proname)
    ...
    # Link Sapphire to the mod
    target_link_libraries(${proname} sapphire)

    if (GEODE_DISABLE_CLI_CALLS)
        message("Skipping setting up sapphire mod ${proname}")
        return()
    endif()

    if(GEODE_CLI STREQUAL "GEODE_CLI-NOTFOUND")
        message(FATAL_ERROR
            "setup_sapphire_mod called, but Sapphire CLI was not found - "
    ...
    # Ensure that the list of mods being built is global (persists between setup_sapphire_mod calls)
    ...
    if (HAS_HEADERS)
        add_custom_target(${proname}_PACKAGE ALL
            ...
            COMMAND ${GEODE_CLI} package new ${CMAKE_CURRENT_SOURCE_DIR} 
                ...
                --output ${CMAKE_CURRENT_BINARY_DIR}/${MOD_ID}.sapphire
                ${INSTALL_ARG}
    ...
    if (EXISTS "${CMAKE_CURRENT_BINARY_DIR}/sapphire-deps")

        file(GLOB dirs ${CMAKE_CURRENT_BINARY_DIR}/sapphire-deps/*)
        ...
        foreach(dir ${dirs})
            if (IS_DIRECTORY ${dir})
                ...
                if (${GEODE_CLI_VERSION} VERSION_GREATER_EQUAL "1.4.1")
                    # Read dep info
                    file(READ "${dir}/sapphire-dep-options.json" DEP_JSON)
        ...
        # Link libs
        target_include_directories(${proname} PUBLIC "${CMAKE_CURRENT_BINARY_DIR}/sapphire-deps")
...
function(create_sapphire_file proname)
    # todo: deprecate at some point ig
    # message(DEPRECATION
    #     "create_sapphire_file has been replaced with setup_sapphire_mod - "
    #     "please replace the function call"
    # )

    # forward all args
    setup_sapphire_mod(${proname} ${ARGN})
endfunction()

function(package_sapphire_resources proname src dest)
    if (GEODE_DISABLE_CLI_CALLS)
    ...
    if(GEODE_CLI STREQUAL "GEODE_CLI-NOTFOUND")
        message(WARNING
            "package_sapphire_resources called, but Sapphire CLI was "
...
function(package_sapphire_resources_now proname src dest header_dest)
    if (GEODE_DISABLE_CLI_CALLS)
        message(WARNING
            "package_sapphire_resources_now called, but GEODE_DISABLE_CLI_CALLS 
    ...
    if(GEODE_CLI STREQUAL "GEODE_CLI-NOTFOUND")
        message(FATAL_ERROR
            "package_sapphire_resources_now called, but Sapphire CLI "
            "was not found - Please install Sapphire CLI from "
            "https://github.com/KWHYTHUB/cli/releases/latest"
        )
        return()
    endif()
    ...
    foreach(file ${RESOURCE_FILES})
        ...
        if (NOT FILE_NAME STREQUAL ".sapphire_cache" AND NOT FILE_SHOULD_HASH EQUAL -1)
